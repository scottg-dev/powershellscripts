...
#$UstnPath = $UstnExe.Substring(0,10)
...
if(!(Test-Path $x))
{
New-Item -ItemType Directory -Force -Path $x
}

#endregion CreateDirs

#region CreateLicFiles

#Rename site.cfg file to eliminate potential confilcts.
$x = $UstnSystemCfgDir + 'site.cfg'
Rename-Item -Path $x -NewName 'site.cfg.save'

#Create the Microstation license file
#$UstnLicenseDir   = $UstnPath + 'Program\Licensing\'
#$UstnLicenseFile  = $UstnPath + 'Program\Licensing\msj.lic'

#This will need a Microstation J license parameter. $microstationjlicserver
#Add-Content $UstnLicenseFile $microstationjlicserver

#This will need a IRAS/B license parameter. $irasblic
#$IRASBLicenseFile  = $UstnPath + 'Program\Licensing\irasb.lic'
#Add-Content $UstnLicenseFile $irasblic

#endregion CreateLicFiles
...
$itemstocopy = $EdeWkspCellDir +'\*'
#$itemstocopy = 'C:\Temp\Temp1\*'
$destination = @($X1,$X2,$X3,$X4,$X5,$X6)
#$destination = @('C:\Temp\Temp2','C:\Temp\Temp3','C:\Temp\Temp4','C:\Temp\Temp5','C:\Temp\Temp6')
foreach($dir in $destination)
{
Copy-Item -Path $itemstocopy -Destination $dir
}
...
$itemstocopy = $UstnSysPenTableDir +'\*'
$destination = @($X1,$X2,$X3,$X4,$X5,$X6)
foreach($dir in $destination)
{
Copy-Item -Path $itemstocopy -Destination $dir
...
$itemtocopy = $EdeWkspDataDir +'\batchplt.spc'
$destination = @($X1,$X2,$X3,$X4,$X5,$X6,$X7)
foreach($dir in $destination)
{
Copy-Item -Path $itemtocopy -Destination $dir
}
...
$itemstocopy = $EdeWkspIntfDir +'\*'
$destination = @($X1,$X2,$X3,$X4,$X5)
foreach($dir in $destination)
{
Copy-Item -Path $itemstocopy -Destination $dir
}
...
$itemstocopy = $EdeWkspFkeysDir +'\*'
$destination = @($X1,$X2,$X3,$X4)
foreach($dir in $destination)
{
Copy-Item -Path $itemstocopy -Destination $dir
}
...
Add-Content -Path $UstnEDECfgFile -Value "EDECLIENTDIR = + $EDEClientDir"
                                         
Add-Content -Path $UstnEDECfgFile -Value "MS_DEF            = + $DiscPath + \                        # Default dirs."
Add-Content -Path $UstnEDECfgFile -Value "MS_BACKUP         = + $WorkDir  + \                        # Backup file location"
Add-Content -Path $UstnEDECfgFile -Value "MS_APP            = $(_USTN_PROJECT)ede/ucm/               # UCM path for TSK."
Add-Content -Path $UstnEDECfgFile -Value "MS_APPMEN         = $(_USTN_PROJECT)ede/data/              # Application menus."
Add-Content -Path $UstnEDECfgFile -Value "MS_CELL           = $(_USTN_PROJECT)ede/cell/              # Cell libraries."
Add-Content -Path $UstnEDECfgFile -Value "MS_CELLLIST       = $(_USTN_PROJECT)ede/cell/*.cel         # Cell libraries."
Add-Content -Path $UstnEDECfgFile -Value "MS_CELLSELECTORDIR= $(_USTN_PROJECT)ede/cell/              # Cell selector file location."
Add-Content -Path $UstnEDECfgFile -Value "MS_FKEYMNU        = $(_USTN_PROJECT)ede/data/funckey.mnu   # Funtion key menu."
Add-Content -Path $UstnEDECfgFile -Value "MS_GLOSSARY       = $(_USTN_PROJECT)ede/data/*.gls         # Glossary files."
Add-Content -Path $UstnEDECfgFile -Value "MS_LEVELNAMES     = $(_USTN_PROJECT)ede/data/              # Level name files."
Add-Content -Path $UstnEDECfgFile -Value "MS_MACRO          < $(_USTN_PROJECT)ede/macros/            # Macro files."
Add-Content -Path $UstnEDECfgFile -Value "MS_PLTFILES       = + $WorkDir  + \                        # Plot file (output)  location."
Add-Content -Path $UstnEDECfgFile -Value "MS_BATCHPLT_SPECS = $(_USTN_PROJECT)ede/data/batchplt.spc  # Batch plotting specifications file"
Add-Content -Path $UstnEDECfgFile -Value "MS_RFDIR          =                                        # Ref file path. (undefined)"
Add-Content -Path $UstnEDECfgFile -Value "MS_SETTINGS       = $(_USTN_PROJECT)ede/data/styles.stg    # Settings file."
Add-Content -Path $UstnEDECfgFile -Value "MS_SETTINGSDIR    = $(_USTN_PROJECT)ede/data/              # Settings resources."
Add-Content -Path $UstnEDECfgFile -Value "MS_SEEDFILES      = $(_USTN_PROJECT)ede/seed/              # Seed files."
Add-Content -Path $UstnEDECfgFile -Value "MS_SHEETSEED      = $(_USTN_PROJECT)ede/seed/transseed.dgn # Sheet seed file."
Add-Content -Path $UstnEDECfgFile -Value "MS_SCR            = $(temp)                                # Temp file location."
Add-Content -Path $UstnEDECfgFile -Value "#MS_SYMBRSRC   Defined in ede.ucf'
Add-Content -Path $UstnEDECfgFile -Value "MS_TMP            = $(temp)                                # Temp file location."
Add-Content -Path $UstnEDECfgFile -Value "MS_TRANSSEED      = $(_USTN_PROJECT)ede/seed/transseed.dgn # DWG seed file."
Add-Content -Path $UstnEDECfgFile -Value "MS_UCM            < $(_USTN_PROJECT)ede/ucm/               # UCM path."
Add-Content -Path $UstnEDECfgFile -Value "MS_WORKSPACEOPTS  = 2                                      # Disallow selection of workspace. "
Add-Content -Path $UstnEDECfgFile -Value "   # (0=allow 1=disabled, displayed 2=disabled, not displayed)"
Add-Content -Path $UstnEDECfgFile -Value "MS_SYMBRSRC       > $(_USTN_PROJECT)ede/symb/*.rsc         # Font resource files."
                                         
Add-Content -Path $UstnEDECfgFile -Value "P_UCM             = $(_USTN_PROJECT)ede/ucm/                                # Parsons UCM path."
Add-Content -Path $UstnEDECfgFile -Value "JOB_CEL           = + $ProjStdDiscPath + $DiscDirname + $ProjAbbr + .cel    # Disc. Tmp cell lib."
...
Add-Content -Path $UstnEDECfgFile -Value $string | Sort-Object -Property $key
}
#2
...
Add-Content -Path $UstnEDECfgFile -Value "PS=$projPath\$projstdpath\$ProjStdGEPath\ #  Project standard GE."
Add-Content -Path $UstnEDECfgFile -Value "PJ=$projPath\$projstdpath\$ProjStdDiscPath\ #  Project standard Disc."
Add-Content -Path $UstnEDECfgFile -Value "CSTD=W:\$CmpyStdGEpath\\ #  Company standards STD GE."
Add-Content -Path $UstnEDECfgFile -Value "DSTD=W:\$CmpyStdDiscPath\\ #  Company standards STD Disc."

#ForAllDiscOneLineNLF( $DiscDirname)
#5
foreach ($key in $dict.values)
{
$string = $projPath + "\" + $SubpDirname + "\" + $key + ";"
$ForAllDisc += $string
}
Add-Content -Path $UstnEDECfgFile -Value $ForAllDisc
#endregion AppendingFile
#endregion MicroStationVariables

#region AppendFile
# Append the project variables to the config file.
$X = $ProjStdGEPath + $ProjUstnWkspDir + $ProjAbbr + '.pcf'
Add-Content $X $UstnEDECfgFile

# Append the disipline variables (project & company stds) to the config file.
$X = $CmpyStdDiscPath + $ProjUstnWkspDir + $DiscAbbr + '.dcf'
Add-Content $X $UstnEDECfgFile


$X = $ProjStdDiscPath + $ProjUstnWkspDir + $DiscAbbr + '.dcf'
Add-Content $X $UstnEDECfgFile

#run(wait, cmd /c copy $UstnEDECfgFile $USTNConfigFile)
Copy-Item $UstnEDECfgFile $USTNConfigFile
