...
#Set parameters for testing.
  $ProjAbbr = "SMG-PROJ"
  $EDEClientWorkDir = "C:\ProgramData\Appsetra"
  ...
  #$Cmpystdpath = "3DStds"
  $CmpyStdGEPath = "W:\ge"
  $CmpyStdDiscPath = "W:\pd"
  $projPath = "S:"
  $ProjStdGEPath = "S:\3DStds\ge"
  $ProjStdDiscPath = "S:\3DStds\pd"
  $projstdpath = "3d"
...
Catch
{
 #WriteToLog $Error "Error with RoboCopy in Section 1 (Copy Cell Libraries)"
}
}
##Section 2
## ----Copy Data Files
...
Catch
{
 #WriteToLog $Error "Error with RoboCopy in Section 2 (Copy Data Files)"
}
}
##Section 3
## ----Copy Seed Files
...
Catch
{
 #WriteToLog $Error "Error with RoboCopy in Section 3 (Copy Seed Files)"
}
}
##Section 4
## ----Copy Symbolgy Files
...
Catch
{
 #WriteToLog $Error "Error with RoboCopy in Section 4 (Copy Symbolgy Files)"
}
}
##Section 5
## ----Copy UCMs
...
Catch
{
 #WriteToLog $Error "Error with RoboCopy in Section 5 (Copy UCMs)"
}
}
##Section 6
## ----Copy Macros
...
Catch
{
 #WriteToLog $Error "Error with RoboCopy in Section 6 (Copy Macros)"
}
}
...
Catch
{
 #WriteToLog $Error "Error with RoboCopy in Section 7 (Copy Plot driver files)"
}
}
...
Catch
{
 #WriteToLog $Error "Error with RoboCopy in Section 8 (Copy Pen Table files)"
}
}
...
Catch
{
 #WriteToLog $Error "Error with RoboCopy in Section 9 (Copy the batch plotting spec file)"
}
}
...
Catch
{
 #WriteToLog $Error "Error with RoboCopy in Section 10 (Copy User Interface Files)"
}
}
...
Catch
{
 #WriteToLog $Error "Error with RoboCopy in Section 11 (Copy function key menus)"
}
}
...
Add-Content -Path $UstnEDECfgFile -Value "MS_DEF            = $projPath\$SubpDirname\$DiscAbbr\\                        # Default dirs."
...
if(Test-Path $X)
{
Write-Host "The proj dcf File Exist"
Add-Content -Path $UstnEDECfgFile -Value $X
}
