...
$AppsetraVariables = "The Environment Variables are Apps Net Dir = $EdeAppsNetDir, Client work Dir = $EDEClientWorkDir, Proj Drive Letter = $ProjectDriveLetter, Proj GE Path = $ProjStdGEPath, Proj std disc path = $ProjStdDiscPath, App Group Appbr = $AppGroupAbbr, App Abbr = $AppAbbr, App Version = $AppVersion"
...
if (!(Test-Path $s))
{
Copy-Item -Path $s -Destination 'C:\win32app\ingr\PDSHELL\scotttesting'
Rename-Item -Path $s -NewName upversionscott.fb
}